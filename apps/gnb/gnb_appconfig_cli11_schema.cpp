...
void srsran::configure_cli11_with_gnb_appconfig_schema(CLI::App& app, gnb_appconfig& gnb_parsed_cfg)
{
  ...
  add_option(app, "--csi-logger-period-slots", gnb_cfg.csi_logger_period_slots, "CSI Logger report period in slots")->capture_default_str();
  app.add_flag("--csi-logger-all-subcarriers", gnb_cfg.csi_logger_all_subcarriers, "CSI Logger: log all subcarriers (true) or 1 per PRB (false)")->capture_default_str();
